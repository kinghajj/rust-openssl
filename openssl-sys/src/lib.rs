...
static mut LOCKS: *mut Vec<StaticMutexGuard> = 0 as *mut Vec<StaticMutexGuard>;

extern fn locking_function(mode: c_int, n: c_int, _file: *const c_char,
                               _line: c_int) {
    unsafe {
        let mutex = &(*MUTEXES)[n as uint];
        let lock = &mut (*LOCKS)[n as uint];
...
pub fn init() {
    ...
    unsafe {
        INIT.doit(|| {
            ...
            let locks = box Vec::from_fn(num_locks as uint, |_| mem::zeroed::<StaticMutexGuard>());
            MUTEXES = mem::transmute(mutexes);
            LOCKS = mem::transmute(locks);
